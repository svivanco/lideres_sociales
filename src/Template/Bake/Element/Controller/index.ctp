...
%>
    ...
    public function getData()
    {
         <%
        $tmp = array();
        $campos = $modelObj->getSchema();
        $fields = $campos->columns();
        %>$aColumns = array
                        (
                            <%
                            foreach($fields as $cmp)
                            {
                                %>
'<%= $currentModelName.".".$cmp %>',
                                <%
                            }
                            %>
);
        
        $sIndexColumn = "<%= $currentModelName %>.id";
...
if(!empty($this->paginate['conditions']))
        {
            $query = $this-><%= $currentModelName %>->find()->where($this->paginate['conditions']);
            
             $this->paginate['contain'] = [<%= $this->Bake->stringifyList($belongsTo, ['indent' => false]) %>];
        ...
        else
        {
            ...
            $<%= $pluralName %> = $this->paginate('<%= $currentModelName %>');
        }
        
        //Numero total de registros
        $iTotalDisplayRecords = $this->request->params['paging']['<%= $currentModelName %>']['count'];

        //Numero de registros encontrados
        $iTotalRecords = $this->request->params['paging']['<%= $currentModelName %>']['current'];
        ...
        foreach($<%= $pluralName %> as $<%= $singularName %>)
        {
            ...
            $actions = "<div class='btn-group' role='group'>";
                $actions .= $Html->link("<i class='icon md-eye' aria-hidden='true'></i>",array('action'=>'view',$<%= $singularName %>->id),array('escape'=>false,'class'=>"btn btn-default"));
                $actions .= $Html->link("<i class='icon md-edit' aria-hidden='true'></i>",array('action'=>'edit',$<%= $singularName %>->id),array('escape'=>false,'class'=>"btn btn-default"));
                $actions .= $Form->postLink("<i class='icon md-delete' aria-hidden='true'></i>", ['action' => 'delete',$<%= $singularName %>->id], ['escape'=>false,'class'=>"btn btn-danger",'confirm' => __('Realmente desea eliminar el registro con el Id # {0}?', $<%= $singularName %>->id)]);
            $actions .="</div>";
            <%
            foreach($fields as $cmp)
            {
	%>
	$aaData[$i][] = $<%= $singularName %>-><%= $cmp %>;
	<%
            }
            %>
            $aaData[$i][] = $actions;
