...
class CatCargosController extends AppController
{
    ...
    public function getData()
    {
        ...
        {
            $query = $this->CatCargos->find()->where($this->paginate['conditions']);
            
             $this->paginate['contain'] = [];
            $catCargos = $this->paginate($query);  
        }
        else
        {
        	$this->paginate['contain'] = [];
            
            $catCargos = $this->paginate('CatCargos');
        }
        
        //Numero total de registros
        $iTotalDisplayRecords = $this->request->params['paging']['CatCargos']['count'];

        //Numero de registros encontrados
        $iTotalRecords = $this->request->params['paging']['CatCargos']['current'];
        ...
        $i = 0;
        foreach($catCargos as $catCargo)
        {

            $actions = "<div class='btn-group' role='group'>";
                $actions .= $Html->link("<i class='icon md-eye' aria-hidden='true'></i>",array('action'=>'view',$catCargo->id),array('escape'=>false,'class'=>"btn btn-default"));
                $actions .= $Html->link("<i class='icon md-edit' aria-hidden='true'></i>",array('action'=>'edit',$catCargo->id),array('escape'=>false,'class'=>"btn btn-default"));
                $actions .= $Form->postLink("<i class='icon md-delete' aria-hidden='true'></i>", ['action' => 'delete',$catCargo->id], ['escape'=>false,'class'=>"btn btn-danger",'confirm' => __('Realmente desea eliminar el registro con el Id # {0}?', $catCargo->id)]);
            $actions .="</div>";
            	$aaData[$i][] = $catCargo->id;
		$aaData[$i][] = $catCargo->name;
		$aaData[$i][] = $catCargo->activo;
		$aaData[$i][] = $catCargo->created;
		$aaData[$i][] = $catCargo->modified;
	            $aaData[$i][] = $actions;
            $i++;
        }
     ...
     * View method
     *
     * @param string|null $id Cat Cargo id.
     * @return \Cake\Http\Response|null
     * @throws \Cake\Datasource\Exception\RecordNotFoundException When record not found.
     */
    public function view($id = null)
    {
        $catCargo = $this->CatCargos->get($id, [
            'contain' => ['CatPersonas']
        ]);

        $this->set('catCargo', $catCargo);
        $this->set('_serialize', ['catCargo']);
    }
    ...
    public function add()
    {
        $catCargo = $this->CatCargos->newEntity();
        if ($this->request->is('post')) {
            $catCargo = $this->CatCargos->patchEntity($catCargo, $this->request->getData());

            if ($this->CatCargos->save($catCargo)) 
            {
        ...
        $catCargos = $this->CatCargos->CatCargosPertenece->find('list');

        $this->set(compact('catCargo','catCargos'));
        $this->set('_serialize', ['catCargo']);
    }
    ...
    public function delete($id = null)
    {
        ...
        $catCargo = $this->CatCargos->get($id);
        if ($this->CatCargos->delete($catCargo)) 
        {
