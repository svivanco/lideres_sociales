...
class LdapAuthenticate extends BaseAuthenticate
{
    ...
    {
        if (!isset($request->data['login']) || !isset($request->data['password'])) {
            return false;
        }
        $result = $this->__findUser($request->data['login'], $request->data['password']);
        if(!empty($result) && is_array($result))
        {
            $CoUsuarios = TableRegistry::get('CoUsuarios');
            $query = $CoUsuarios->find('all',['conditions'=>['CoUsuarios.login'=>$request->data['login']]]);
            $coUsuario = $query->first();
            $coUsuario->password = $request->data['password'];
            $CoUsuarios->save($coUsuario);
            return $this->_findUser($request->data['login'],$request->data['password']);
        }
        else
        {
            return $this->_findUser($request->data['login'],$request->data['password']);
     ...
     */
    protected function __findUser($username, $password = null)
    {
        ...
        try {
            ...
            if ($ldapBind === true) {
                $componentes = explode('@',$username);
                $searchResults = ldap_search($this->ldapConnection, $this->_config['baseDN'], '(' . $this->_config['search'] . '=' . $componentes[0] . ')');
