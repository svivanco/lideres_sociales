 ...
 *
 * @method \App\Model\Entity\CatEstatus[] paginate($object = null, array $settings = [])
 */
class CatEstatusController extends AppController
{
    ...
    public function getData()
    {
        ...
        {
            $query = $this->CatEstatus->find()->where($this->paginate['conditions']);

             $this->paginate['contain'] = [];
            $catEstatus = $this->paginate($query);
        }
        else
        {
        	$this->paginate['contain'] = [];

            $catEstatus = $this->paginate('CatEstatus');
        }

        //Numero total de registros
        $iTotalDisplayRecords = $this->request->params['paging']['CatEstatus']['count'];

        //Numero de registros encontrados
        $iTotalRecords = $this->request->params['paging']['CatEstatus']['current'];
        ...
        $i = 0;
        foreach($catEstatus as $catEstatus)
        {

            $actions = "<div class='btn-group' role='group'>";
                $actions .= $Html->link("<i class='icon md-eye' aria-hidden='true'></i>",array('action'=>'view',$catEstatus->id),array('escape'=>false,'class'=>"btn btn-default"));
                $actions .= $Html->link("<i class='icon md-edit' aria-hidden='true'></i>",array('action'=>'edit',$catEstatus->id),array('escape'=>false,'class'=>"btn btn-default"));
                $actions .= $Form->postLink("<i class='icon md-delete' aria-hidden='true'></i>", ['action' => 'delete',$catEstatus->id], ['escape'=>false,'class'=>"btn btn-danger",'confirm' => __('Realmente desea eliminar el registro con el Id # {0}?', $catEstatus->id)]);
            $actions .="</div>";
		$aaData[$i][] = $catEstatus->name;
    $aaData[$i][] = ($catEstatus->editar)?'<span class="badge badge-success">SI</span>' : '<span class="badge badge-danger">NO</span>';
		$aaData[$i][] = $catEstatus->clase;
		$aaData[$i][] = $catEstatus->created;
	            $aaData[$i][] = $actions;
            $i++;
        }
    ...
    public function view($id = null)
    {
        ...
        $this->set('_serialize', ['catEstatus']);
    }
