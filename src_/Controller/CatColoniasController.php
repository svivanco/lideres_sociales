...
/**
 * CatColonias Controller
 *
 * @property \App\Model\Table\CatColoniasTable $CatColonias
 *
 * @method \App\Model\Entity\CatColonia[] paginate($object = null, array $settings = [])
 */
class CatColoniasController extends AppController
{
    ...
    public function getData()
    {
         $aColumns = array
                        (
                            'CatColonias.id',
                                'CatColonias.codigo_postal',
                                'CatColonias.cat_estado_id',
                                'CatColonias.cat_municipio_id',
                                'CatColonias.ciudad',
                                'CatColonias.tipo_asentamiento',
                                'CatColonias.asentamiento',
                                'CatColonias.clave_oficina',
                                );
        
        $sIndexColumn = "CatColonias.id";
        ...
        {
            $query = $this->CatColonias->find()->where($this->paginate['conditions']);
            
             $this->paginate['contain'] = [];
            $catColonias = $this->paginate($query);  
        }
        else
        {
        	$this->paginate['contain'] = [];
            
            $catColonias = $this->paginate('CatColonias');
        }
        
        //Numero total de registros
        $iTotalDisplayRecords = $this->request->params['paging']['CatColonias']['count'];

        //Numero de registros encontrados
        $iTotalRecords = $this->request->params['paging']['CatColonias']['current'];
        ...
        $i = 0;
        foreach($catColonias as $catColonia)
        {

            $actions = "<div class='btn-group' role='group'>";
                $actions .= $Html->link("<i class='icon md-eye' aria-hidden='true'></i>",array('action'=>'view',$catColonia->id),array('escape'=>false,'class'=>"btn btn-default"));
                $actions .= $Html->link("<i class='icon md-edit' aria-hidden='true'></i>",array('action'=>'edit',$catColonia->id),array('escape'=>false,'class'=>"btn btn-default"));
                $actions .= $Form->postLink("<i class='icon md-delete' aria-hidden='true'></i>", ['action' => 'delete',$catColonia->id], ['escape'=>false,'class'=>"btn btn-danger",'confirm' => __('Realmente desea eliminar el registro con el Id # {0}?', $catColonia->id)]);
            $actions .="</div>";
            	$aaData[$i][] = $catColonia->id;
		$aaData[$i][] = $catColonia->codigo_postal;
		$aaData[$i][] = $catColonia->cat_estado_id;
		$aaData[$i][] = $catColonia->cat_municipio_id;
		$aaData[$i][] = $catColonia->ciudad;
		$aaData[$i][] = $catColonia->tipo_asentamiento;
		$aaData[$i][] = $catColonia->asentamiento;
		$aaData[$i][] = $catColonia->clave_oficina;
	            $aaData[$i][] = $actions;
            $i++;
        }
     ...
     * View method
     *
     * @param string|null $id Cat Colonia id.
     * @return \Cake\Http\Response|null
     * @throws \Cake\Datasource\Exception\RecordNotFoundException When record not found.
     */
    public function view($id = null)
    {
        $catColonia = $this->CatColonias->get($id, [
            'contain' => []
        ]);

        $this->set('catColonia', $catColonia);
        $this->set('_serialize', ['catColonia']);
    }
    ...
    public function add()
    {
        $catColonia = $this->CatColonias->newEntity();
        if ($this->request->is('post')) {
            $catColonia = $this->CatColonias->patchEntity($catColonia, $this->request->getData());
            if ($this->CatColonias->save($catColonia)) 
            {
    ...
    public function delete($id = null)
    {
        ...
        $catColonia = $this->CatColonias->get($id);
        if ($this->CatColonias->delete($catColonia)) 
        {
    ...
    public function getColoniasPorMunicipio()
    {
     if ($this->request->is('post'))
     {
       $catColonias = $this->CatColonias->find('list',['conditions'=>[
                                                                               'CatColonias.cat_municipio_id'=>$this->request->data['id']
                                                                               ]]);
     }
     header('Content-Type: application/json');
     echo json_encode($catColonias);
     exit();
    }
